...
def log_marg(x,y_in,lbda_bounds,sigma_bounds):
  y=y_in-np.mean(y_in)
  n=x.shape[0]
  ...
  best_lbda=None
  best_sigma=None
  for lbda_seed in np.geomspace(1.1*lbda_bounds[0],0.9*lbda_bounds[1],5):
    for sigma_seed in np.geomspace(1.1*sigma_bounds[0],0.9*sigma_bounds[1],5):
...
def make_data(seed):
  ...
  x_tr=np.random.exponential(1,N_OBS).reshape((-1,1))
  #x_tr=np.random.pareto(1,N_OBS).reshape((-1,1))
  #x_tr=np.abs(np.random.normal(0,1,N_OBS)).reshape((-1,1))
...
def make_data(seed):
  ...
  N_TR=50
  def fy(x):
    return np.sin(3*2*np.pi*x**.5)
  x_tr=np.abs(np.random.normal(0,1,N_TR)).reshape((-1,1))
  y_tr=fy(x_tr)+np.random.normal(0,.1,x_tr.shape)
  x_val=np.random.uniform(np.min(x_tr),np.max(x_tr),NS).reshape((-1,1))
  #x_val=np.abs(np.random.normal(0,1,NS)).reshape((-1,1))
  y_val=fy(x_val)
  return x_tr, y_tr, x_val, y_val
...
def make_data(seed):
  ...
  def fy(x):
    return np.sin(3*2*np.pi*x**.5)
  x_tr=np.abs(np.random.normal(0,2,N_TR)).reshape((-1,1))
  y_tr=fy(x_tr)+np.random.normal(0,.1,x_tr.shape)
  #x_val=np.random.uniform(np.min(x_tr),np.max(x_tr),NS).reshape((-1,1))
  x_val=np.abs(np.random.normal(0,2,NS)).reshape((-1,1))
  y_val=fy(x_val)
  return x_tr, y_tr, x_val, y_val
...
for seed in range(100):
  ...
  lbda, sigma=log_marg(x_tr,y_tr, lbda_bounds=[1e-6,10],sigma_bounds=[1e-6,10])
