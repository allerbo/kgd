...
def kgd(Xs,X_tr,y_tr_in,sigma0=None,step_size=0.01, sigma_min=0, t_max=1e4, plot=False, sleep_time=0.1, val_data=None, path=False):
  if plot:
  ...
  sigma=3*np.sqrt(np.max(D2)) if sigma0 is None else sigma0
  ...
  for i in range(int(t_max/step_size)):
    ...
    if r2>0.9999 or (plot and i%100/step_size==0):
      ...
      if r2>0.9999:
        break
  if path:
    return ys+y_tr_mean, sigmas, r2s
  return ys+y_tr_mean
